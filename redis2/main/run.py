...
async def lpush(host, port, db, password, key, value):
    try:
        import redis
    except:
        logger.info("[Redis] 导入 redis 模块失败, 请输入命令 pip install redis")
    ...
    return {"status": 0, "result": r.lpush(key, value)}

async def rpop(host, port, db, password, key):
    try:
        import redis
    except:
        logger.info("[Redis] 导入 redis 模块失败, 请输入命令 pip install redis")
    ...
    return {"status": 0, "result": r.rpop(key)}


async def lrange(host, port, db, password, key, index1, index2):
    try:
        import redis
    except:
        logger.info("[Redis] 导入 redis 模块失败, 请输入命令 pip install redis")
    ...
    logger.info("[Redis] APP执行参数为: {host} {port} {db} {password} {key} {index1} {index2}", host=host, port=port, db=db,
                password=password,
                key=key, index1=index1, index2=index2)
    ...
    return {"status": 0, "result": r.lrange(key, index1, index2)}


async def llen(host, port, db, password, key):
    try:
        import redis
    except:
        logger.info("[Redis] 导入 redis 模块失败, 请输入命令 pip install redis")
    ...
    return {"status": 0, "result": r.llen(key)}
